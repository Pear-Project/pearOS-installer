"""Load bundled TTF fonts (Sen, Borel) without installing them system-wide.

Equivalent of the @font-face rules in assistant-styles.css / aa/style.css.
Uses the standard Linux trick of registering a font file directly with
fontconfig (FcConfigAppFontAddFile), so Pango/Cairo can address it by family
name immediately, in-process, without touching /usr/share/fonts.
"""
import ctypes
import ctypes.util
import os
...
BOREL_FONT = os.path.join(FONTS_DIR, "Borel-Regular.ttf")
