...
{
    // pearOS: the window border theme's X button is meant to minimize
    // every app (matching macOS), not quit it - but Calamares' own close
    // handling used to run ViewManager::quit()'s cancel-confirmation flow
    // here, which could bypass that and actually terminate the install.
    // An installer mid-run has no business being closeable by a titlebar
    // click at all, so this now always minimizes instead, unconditionally
    // - there's no supported "quit via the window controls" path left.
    event->ignore();
    showMinimized();
}
