...
PartitionLabelsView::drawLabel( QPainter* painter,
                                ...
                                bool selected )
{
    // Qt::black was hardcoded here, unreadable against this branding's dark
    // background (this widget paints its own text directly, so the QSS
    // "color:" rules elsewhere in the app can't reach it) - the palette's
    // own window-text color already resolves correctly against whatever
    // background this is drawn on.
    const QColor labelTextColor = palette().color( QPalette::WindowText );
