    ...
    , m_isEfi( PartUtils::isEfiSystem() )
{
    ...
    // The "Storage device:" row is now redundant - the sidebar (below) is
    // the real device picker. deviceComboBox stays alive and functional
    // (every method in this file still reads its currentIndex() as the
    // source of truth, synced from the sidebar), just not shown.
    m_ui->label_2->setVisible( false );
    m_ui->deviceComboBox->setVisible( false );

    // macOS Disk Utility-style sidebar: a hierarchical (disk -> partitions)
    // tree over the exact same deviceModel deviceComboBox already uses -
    // no new data source. deviceComboBox stays the single source of truth
    // every other method in this file already reads from; selecting any
    // row (device or partition) in the tree just moves its current index.
    setupSidebarViewMenu();
    rebuildDeviceSidebar();
    connect( m_ui->deviceSidebarView,
             &QAbstractItemView::clicked,
             this,
             [ this ]( const QModelIndex& index )
             {
                 int row = index.data( Qt::UserRole + 1 ).toInt();
                 if ( row != m_ui->deviceComboBox->currentIndex() )
                 {
                     m_ui->deviceComboBox->setCurrentIndex( row );
                 }
             } );

    // Disk Utility-style toolbar look for the existing action buttons -
    // icons only, every slot/connection below stays exactly as it was.
    m_ui->newPartitionTableButton->setIcon( style()->standardIcon( QStyle::SP_DriveHDIcon ) );
    m_ui->createButton->setIcon( style()->standardIcon( QStyle::SP_FileDialogNewFolder ) );
    m_ui->editButton->setIcon( style()->standardIcon( QStyle::SP_FileDialogDetailedView ) );
    m_ui->deleteButton->setIcon( style()->standardIcon( QStyle::SP_TrashIcon ) );
    m_ui->revertButton->setIcon( style()->standardIcon( QStyle::SP_DialogResetButton ) );
    for ( QPushButton* button :
          { m_ui->newPartitionTableButton, m_ui->createButton, m_ui->editButton, m_ui->deleteButton } )
    {
...
void
PartitionPage::setupSidebarViewMenu()
{
    auto* menu = new QMenu( m_ui->viewMenuButton );
    auto* group = new QActionGroup( menu );
    group->setExclusive( true );

    auto* onlyVolumes = menu->addAction( tr( "Show Only Volumes" ) );
    onlyVolumes->setCheckable( true );
    onlyVolumes->setChecked( true );
    group->addAction( onlyVolumes );

    auto* allDevices = menu->addAction( tr( "Show All Devices" ) );
    ...
    m_ui->viewMenuButton->setMenu( menu );
}

void
PartitionPage::rebuildDeviceSidebar()
{
    auto* model = new QStandardItemModel( m_ui->deviceSidebarView );
    ...
    for ( int row = 0; row < deviceModel->rowCount(); ++row )
    {
        ...
        if ( device->partitionTable() )
        {
            ...
            {
                if ( !Calamares::Partition::isPartitionFreeSpace( p ) )
                {
                    realPartitions << p;
                }
            }
    ...
    m_ui->deviceSidebarView->setModel( model );
    m_ui->deviceSidebarView->expandAll();
    if ( model->rowCount() > 0 )
    {
        QModelIndex firstIndex = model->index( 0, 0 );
        m_ui->deviceSidebarView->setCurrentIndex( firstIndex );
...
PartitionPage::updateFromCurrentDevice()
{
    ...
    // macOS Disk Utility-style info grid - purely derived from data
    // already exposed by KPMcore (Device::capacity(), Partition::used(),
    // PartitionTable::tableTypeToName()), no new state, nothing that
    // affects any job/action logic below.
    {
        ...
        const QString connectionText = diskInfoConnection( device->deviceNode() );
        m_ui->locationValueLabel->setText( diskInfoLocation( device->deviceNode() ) );
        m_ui->connectionValueLabel->setText( connectionText );
        m_ui->partitionMapValueLabel->setText(
            device->partitionTable() ? PartitionTable::tableTypeToName( device->partitionTable()->type() )
                                      : tr( "None" ) );
        m_ui->smartValueLabel->setText( diskInfoSmartStatus( connectionText ) );
        m_ui->capacityValueLabel->setText( formatByteSize( device->capacity() ) );
        m_ui->childCountValueLabel->setText( QString::number( partitionCount ) );
        m_ui->typeValueLabel->setText( diskInfoMediaType( device->deviceNode() ) );
        m_ui->deviceValueLabel->setText( device->deviceNode() );
    }
