...
signals:
    ...
    void actionChosen();
    void deviceChosen();
    // Disk Utility-style toolbar UX: picking "Manual Partitioning" from the
    // toolbar should jump straight to the partition editor, without making
    // the user click the wizard's separate Next button afterwards.
    void manualPartitioningRequested();
...
private:
    ...
    // macOS Disk Utility-style sidebar (disk -> partitions tree + View
    // menu). Purely a presentation layer over m_drivesCombo's existing
    // deviceModel - see ChoicePage.cpp::init() for why.
    void setupSidebarViewMenu();
    void rebuildDeviceSidebar();
    bool m_sidebarShowOnlyVolumes = true;

    // Disk Utility-style toolbar shortcuts for the three named choices -
    // each just calls setChecked(true) on the *same* PrettyRadioButton
    // below (m_eraseButton/m_alongsideButton/m_somethingElseButton),
    // triggering the exact same QButtonGroup signal chain and job/action
    // logic. Visibility mirrors those widgets' own show()/hide() calls
    // (see syncToolbarChoiceButtons()), so a toolbar button only appears
    // when the choice it stands for is actually available.
    void setupToolbarChoiceButtons();
    void syncToolbarChoiceButtons();
    QToolButton* m_eraseToolButton = nullptr;
    QToolButton* m_alongsideToolButton = nullptr;
    QToolButton* m_manualToolButton = nullptr;

    // pearOS Disk Utility handoff: written every time the toolbar choice
    // changes, so an external process (the pearOS Python installer, which
    // minimizes this window once a choice is made) can show a
    // confirmation screen of its own without needing any Calamares UI
    // access - see RemoteControl.h for the matching remote-control socket.
    void writeChoiceSnapshot();
