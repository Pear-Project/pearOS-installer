    ...
    , m_enableEncryptionWidget( true )
{
    ...
    // Maximum (not Expanding) - the bar+legend inside are both Fixed-height,
    // so the frame should hug its natural content height instead of
    // stretching to fill the row and pushing diskInfoPanel far down below it.
    m_previewBeforeFrame->setSizePolicy( QSizePolicy::Preferred, QSizePolicy::Maximum );
    ...
    // Disk Utility-style: the choice list below is now redundant with the
    // toolbar buttons (setupToolbarChoiceButtons()) for the three named
    // choices, so hide it and show the disk icon/title/capacity header +
    // info grid instead, matching PartitionPage's own info panel. Known
    // tradeoff: the erase/replace filesystem-type combo boxes (only shown
    // when >1 fs type is configured - an edge case, not this profile's
    // default) live inside the now-hidden PrettyRadioButton rows and
    // become unreachable this way; everything else (encryption, reuseHome,
    // the actual choice logic) is untouched and still works via the
    // toolbar or its own always-visible widget.
    m_itemsScrollArea->hide();
    hLine->hide();
    // "Current:" above the usage bar is a leftover Before/After-preview
    // label from the original list-based UI - Disk Utility's own bar has
    // no such caption above it.
    m_previewBeforeLabel->hide();
    diskHeaderIcon->setPixmap( style()->standardIcon( QStyle::SP_DriveHDIcon ).pixmap( 64, 64 ) );
...
void
ChoicePage::setupSidebarViewMenu()
{
    auto* menu = new QMenu( viewMenuButton );
    auto* group = new QActionGroup( menu );
    group->setExclusive( true );

    auto* onlyVolumes = menu->addAction( tr( "Show Only Volumes", "@action" ) );
    onlyVolumes->setCheckable( true );
    onlyVolumes->setChecked( true );
    group->addAction( onlyVolumes );

    auto* allDevices = menu->addAction( tr( "Show All Devices", "@action" ) );
    allDevices->setCheckable( true );
    group->addAction( allDevices );
    ...
    connect( onlyVolumes,
             ...
             [ this ]( bool checked )
             {
                 if ( checked )
                 {
                     m_sidebarShowOnlyVolumes = true;
                     rebuildDeviceSidebar();
                 }
             } );
    connect( allDevices,
             &QAction::toggled,
             ...
             [ this ]( bool checked )
             {
                 if ( checked )
                 {
                     m_sidebarShowOnlyVolumes = false;
                     rebuildDeviceSidebar();
                 }
             } );

    viewMenuButton->setMenu( menu );
}

void
ChoicePage::rebuildDeviceSidebar()
{
    auto* model = new QStandardItemModel( deviceSidebarView );
    auto* deviceModel = m_core->deviceModel();

    for ( int row = 0; row < deviceModel->rowCount(); ++row )
    {
        QModelIndex deviceIndex = deviceModel->index( row, 0 );
        Device* device = deviceModel->deviceForIndex( deviceIndex );
        if ( !device )
        {
            continue;
        }

        QVector< Partition* > realPartitions;
        if ( device->partitionTable() )
        {
            for ( Partition* p : device->partitionTable()->children() )
            {
                if ( !isPartitionFreeSpace( p ) )
                {
                    realPartitions << p;
                }
            }
        ...
        // "Show Only Volumes" hides devices with nothing real on them -
        // mainly the phantom nbd0..15 device nodes the kernel always
        // creates, which aren't real installable disks (same filtering
        // system_install's own disk_backend.py already applies).
        if ( m_sidebarShowOnlyVolumes && realPartitions.isEmpty() )
        {
            continue;
        }

        auto* deviceItem = new QStandardItem( deviceModel->data( deviceIndex, Qt::DisplayRole ).toString() );
        deviceItem->setEditable( false );
        deviceItem->setData( row, Qt::UserRole + 1 );

        for ( Partition* p : realPartitions )
        {
            QString mountPoint = PartitionInfo::mountPoint( p );
            QString label = !mountPoint.isEmpty() ? mountPoint : p->deviceNode();
            auto* partItem = new QStandardItem( label );
            partItem->setEditable( false );
            partItem->setData( row, Qt::UserRole + 1 );
            deviceItem->appendRow( partItem );
        }

        model->appendRow( deviceItem );
    }

    deviceSidebarView->setModel( model );
    deviceSidebarView->expandAll();
    if ( model->rowCount() > 0 )
    {
        QModelIndex firstIndex = model->index( 0, 0 );
        deviceSidebarView->setCurrentIndex( firstIndex );
        // Keep m_drivesCombo (the real source of truth) in sync with
        // whatever the tree is showing as selected right now - without
        // this, the tree's row 0 (post-filtering) can point at a
        // different device than m_drivesCombo's own still-unfiltered
        // row 0, showing a header/grid for the wrong device.
        int row = firstIndex.data( Qt::UserRole + 1 ).toInt();
        if ( row != m_drivesCombo->currentIndex() )
        {
            m_drivesCombo->setCurrentIndex( row );
        }
    }
}

void
ChoicePage::setupToolbarChoiceButtons()
{
    // Disk Utility-style: Erase Disk / Manual Partitioning / Install
    // Alongside as toolbar buttons, positioned right after the View
    // button. Each just proxies a click to the *existing*
    // PrettyRadioButton's setChecked(true) - same QButtonGroup, same
    // signal chain, same job/action logic as the list below; this is
    // purely an additional way to trigger the same choice, not a new one.
    m_eraseToolButton = new QToolButton( this );
    m_eraseToolButton->setText( tr( "Erase Disk", "@action" ) );
    connect( m_eraseToolButton, &QToolButton::clicked, this, [ this ] { m_eraseButton->setChecked( true ); } );
    topToolbarLayout->insertWidget( 1, m_eraseToolButton );

    m_alongsideToolButton = new QToolButton( this );
    m_alongsideToolButton->setText( tr( "Install Alongside", "@action" ) );
    connect(
        m_alongsideToolButton, &QToolButton::clicked, this, [ this ] { m_alongsideButton->setChecked( true ); } );
    topToolbarLayout->insertWidget( 2, m_alongsideToolButton );

    m_manualToolButton = new QToolButton( this );
    m_manualToolButton->setText( tr( "Manual Partitioning", "@action" ) );
    connect( m_manualToolButton,
             &QToolButton::clicked,
             this,
             [ this ]
             {
                 m_somethingElseButton->setChecked( true );
                 Q_EMIT manualPartitioningRequested();
             } );
    topToolbarLayout->insertWidget( 3, m_manualToolButton );

    syncToolbarChoiceButtons();
}

void
ChoicePage::syncToolbarChoiceButtons()
{
    // isVisible() reflects whether the widget is actually on-screen right
    // now, which is false during init() (the page isn't shown yet) even
    // after show() was called - isHidden() mirrors the explicit show()/hide()
    // state the PrettyRadioButtons were put into, regardless of ancestor
    // visibility, so it stays correct both during init and afterwards.
    if ( m_eraseToolButton )
    {
        m_eraseToolButton->setVisible( !m_eraseButton->isHidden() );
    }
    if ( m_alongsideToolButton )
    {
        m_alongsideToolButton->setVisible( !m_alongsideButton->isHidden() );
    }
    if ( m_manualToolButton )
    {
        m_manualToolButton->setVisible( !m_somethingElseButton->isHidden() );
    }
}

void
ChoicePage::writeChoiceSnapshot()
{
    QJsonObject snapshot;
    snapshot.insert( QStringLiteral( "installChoice" ),
                      Config::installChoiceNames().find( m_config->installChoice() ) );

    Device* currentDevice = selectedDevice();
    if ( currentDevice )
    {
        snapshot.insert( QStringLiteral( "device" ), currentDevice->deviceNode() );
        snapshot.insert( QStringLiteral( "deviceName" ), currentDevice->name() );
    }

    QFile file( QStringLiteral( "/tmp/pearos-disk-utility-choice.json" ) );
    ...
    {
        cWarning() << "ChoicePage: could not write choice snapshot to" << file.fileName();
    }
}
...
ChoicePage::init( PartitionCoreModule* core )
{
    ...
    // macOS Disk Utility-style sidebar: a hierarchical (disk -> partitions)
    // tree built from the exact same deviceModel/PartitionCoreModule data
    // m_drivesCombo already uses - no new data source. m_drivesCombo stays
    // the single source of truth every method in this file already reads
    // from; selecting any row (device or partition) in the tree just moves
    // m_drivesCombo's current index, same as before.
    m_drivesLabel->setVisible( false );
    m_drivesCombo->setVisible( false );
    setupSidebarViewMenu();
    rebuildDeviceSidebar();
    connect( deviceSidebarView,
             &QAbstractItemView::clicked,
             this,
             [ this ]( const QModelIndex& index )
             {
                 int row = index.data( Qt::UserRole + 1 ).toInt();
                 if ( row != m_drivesCombo->currentIndex() )
                 {
                     m_drivesCombo->setCurrentIndex( row );
                 }
...
ChoicePage::updateDeviceStatePreview()
{
    ...
    // Disk Utility-style header + info grid - same computation already
    // verified working in PartitionPage::updateFromCurrentDevice().
    int partitionCount = 0;
    if ( currentDevice->partitionTable() )
    {
        for ( Partition* p : currentDevice->partitionTable()->children() )
        {
            if ( isPartitionFreeSpace( p ) )
            {
                continue;
    ...
    const QString connectionText = diskInfoConnection( currentDevice->deviceNode() );
    const QString locationText = diskInfoLocation( currentDevice->deviceNode() );
    const QString partitionMapText = currentDevice->partitionTable()
        ? PartitionTable::tableTypeToName( currentDevice->partitionTable()->type() )
        : tr( "None" );

    // name(), not prettyName() - prettyName() bakes the capacity into the
    // string too, and that's already shown separately in the badge/grid.
    diskHeaderTitle->setText( currentDevice->name() );
    // e.g. "USB External Disk" / "Apple Fabric Internal Disk", plus the
    // partition map - same wording Disk Utility uses under the device name.
    QString subtitle = tr( "%1 %2 Disk", "@label %1=connection type, %2=Internal/External" )
                            .arg( connectionText, locationText );
    if ( currentDevice->partitionTable() )
    {
        subtitle += tr( ", %1 Partition Map", "@label %1=partition table type name" ).arg( partitionMapText );
    }
    diskHeaderSubtitle->setText( subtitle );
    diskHeaderCapacityBadge->setText( formatByteSize( currentDevice->capacity() ) );

    locationValueLabel->setText( locationText );
    connectionValueLabel->setText( connectionText );
    partitionMapValueLabel->setText( partitionMapText );
    smartValueLabel->setText( diskInfoSmartStatus( connectionText ) );
    capacityValueLabel->setText( formatByteSize( currentDevice->capacity() ) );
    childCountValueLabel->setText( QString::number( partitionCount ) );
    typeValueLabel->setText( diskInfoMediaType( currentDevice->deviceNode() ) );
    deviceValueLabel->setText( currentDevice->deviceNode() );
    ...
    // Disk Utility's legend sits right up against the bottom of the bar.
    layout->setSpacing( 2 );
    ...
    // Both views above are Fixed-height (QSizePolicy::Expanding, Fixed),
    // but m_previewBeforeFrame itself is vertically Expanding to fill the
    // grid row - without an explicit trailing stretch, Qt was splitting
    // that surplus space into the gap between the two views instead of
    // leaving it below them, pushing the legend far from the bar.
