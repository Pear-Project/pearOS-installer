...
PartitionViewStep::continueLoading()
{
    ...
    // Disk Utility-style toolbar UX: the "Manual Partitioning" toolbar
    // button jumps straight into the partition editor instead of making
    // the user click the wizard's separate Next button afterwards.
    connect( m_choicePage, &ChoicePage::manualPartitioningRequested, this, &PartitionViewStep::next );
}
